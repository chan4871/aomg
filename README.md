...
🔸 협업과 코드 병합
프로젝트는 팀원들과 각자 역할을 나눠 진행되었고, 이후 코드 병합 과정에서 다양한 이슈들이 발생했습니다. 이를 통해 다음과 같은 점들을 배울 수 있었습니다:
1.폴더 및 파일 구조 설계의 중요성
2.코드 스타일 가이드의 필요성
3.GitHub와 FileZilla를 통한 협업 시 경로 설정의 정확성
4.유지보수가 용이한 코드 정리 방식
